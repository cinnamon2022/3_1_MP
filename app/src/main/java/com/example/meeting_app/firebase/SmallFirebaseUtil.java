...
public class SmallFirebaseUtil {

    private Activity activity;
    private OnPostListener onPostListener;
    private int successCount;

    public SmallFirebaseUtil(Activity activity) {
    ...
    public void storageDelete(final PostInfo postInfo){
        ...
        for (int i = 0; i < contentsList.size(); i++) {
            ...
            if (isStorageUrl(contents)) {
                successCount++;
                StorageReference desertRef = storageRef.child("small_posts/" + id + "/" + storageUrlToName(contents));
    ...
    private void storeDelete(final String id, final PostInfo postInfo) {
        ...
        if (successCount == 0) {
            firebaseFirestore.collection("small_posts").document(id)
                    .delete()
